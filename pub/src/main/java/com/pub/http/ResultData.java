    ...
    public ResultData parse(String json, Class<T> clazz) {
        ...
        try {
            ...
            if (!jsonObject.isNull("data") && JsonUtil.isGoodJson(jsonObject.getString("data"))) {
                Object oJson = new JSONTokener(jsonObject.getString("data")).nextValue();
                if (oJson instanceof JSONObject) {
                    JSONObject newJsonObject = (JSONObject) oJson;
                    result.setData(JsonUtil.fromJsonString(newJsonObject.getString("data"), clazz));
