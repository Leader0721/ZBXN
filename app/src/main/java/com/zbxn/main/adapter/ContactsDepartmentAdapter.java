...
//        }
    ...
    public ContactsDepartmentAdapter(Activity context, final List<ContactsDepartmentEntity> data, ICustomListener listener
            ...
            holder.mLayout.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mType == 0) {
                        ...
                        intent.putExtra(ContactsDetailActivity.Flag_Input_Contactor, entitySub);
                    ...
                    if (mType == 3) {
                        EventCustom eventCustom = new EventCustom();
                        eventCustom.setTag(KeyEvent.WORKBLOGCONTACTSSELECT);
                        eventCustom.setObj(entitySub);
                        EventBus.getDefault().post(eventCustom);
                    }
                }
            });
    ...
    private static ContactsDepartmentEntity lastItemInfoDepartment = null;

    // 修改选中的状态
    public void changeState(ContactsDepartmentEntity itemInfo) { // 多选时
        // 单选时
