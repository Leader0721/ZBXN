package com.zbxn.main.widget.calendar.views;
...
import com.zbxn.main.widget.calendar.bizs.calendars.ZSSCalendarManager;
import com.zbxn.main.widget.calendar.bizs.decors.DPDecor;
import com.zbxn.main.widget.calendar.bizs.themes.DPTManager;
import com.zbxn.main.widget.calendar.entities.DateInfo;
...
public class MonthView extends View {
    ...
    private int criticalWidth, criticalHeight;//关键宽 高
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(measureWidth, (int) ((measureWidth / 7F - 20) * 6F) + 20);//设置宽减20 * 6行的高度
    }
    ...
    private void draw(Canvas canvas, Rect rect, DateInfo info) {
        ...
        drawGregorian(canvas, rect, info.strG, info.isWeekend, info.isToday, info.isChoosed);
        if (isFestivalDisplay)
            drawFestival(canvas, rect, info.strF, info.isFestival, info.isToday, info.isChoosed);
    ...
    private void drawBGToday(Canvas canvas, Rect rect) {
        ...
        todayPaint.setStrokeWidth(2);//这个地方改线宽,单位是像素
    ...
    private void drawBGSchedule(Canvas canvas, Rect rect, boolean isSchedule, boolean isChoosed) {
        ...
        if (isSchedule) {
            if (isChoosed) {
                mPaint.setColor(mTManager.colorChooseDot());
            } else {
                mPaint.setColor(mTManager.colorUnChooseDot());
            }
            //小点的位置、半径
            canvas.drawCircle(rect.centerX(), rect.centerY() + circleRadius / 3 + yOffset, 5, mPaint);
        }
    }
     ...
     * @param isChoosed
     */
    private void drawGregorian(Canvas canvas, Rect rect, String str, boolean isWeekend, boolean isToday, boolean isChoosed) {
        mPaint.setTextSize(sizeTextGregorian);
        // 自定义一个文本大小
        ...
        canvas.drawText(str, rect.centerX(), y + yOffset, mPaint);
    }
     ...
     * @param isChoosed
     */
    private void drawFestival(Canvas canvas, Rect rect, String str, boolean isFestival, boolean isToday, boolean isChoosed) {
        mPaint.setTextSize(sizeTextFestival);
        if (isFestival) {
            mPaint.setColor(mTManager.colorF());
        } else {
            if (isToday && isTodayDisplay) {//设置今天农历用蓝色
                mPaint.setColor(mTManager.colorTodayText());
            } else {
        ...
        if (str.contains("&")) {
            ...
            if (mPaint.measureText(str1) > rect.width()) {
                ...
                canvas.drawText(str1.substring(0, length), rect.centerX(), rect.centerY() + offsetYFestival1 + yOffset, mPaint);
                canvas.drawText(str1.substring(length), rect.centerX(), rect.centerY() + offsetYFestival2 + yOffset, mPaint);
            } else {
                canvas.drawText(str1, rect.centerX(), rect.centerY() + offsetYFestival1 + yOffset, mPaint);
                String str2 = s[1];
                if (mPaint.measureText(str2) < rect.width()) {
                    canvas.drawText(str2, rect.centerX(), rect.centerY() + offsetYFestival2 + yOffset, mPaint);
                }
            }
        } else {
            if (mPaint.measureText(str) > rect.width()) {
                ...
                int length = (int) (rect.width() / ch);
                canvas.drawText(str.substring(0, length), rect.centerX(), rect.centerY() + offsetYFestival1 + yOffset, mPaint);
                canvas.drawText(str.substring(length), rect.centerX(), rect.centerY() + offsetYFestival2 + yOffset, mPaint);
            } else {
                canvas.drawText(str, rect.centerX(), rect.centerY() + offsetYFestival1 + yOffset, mPaint);
            }
        }
    }
    ...
    private BGCircle createCircle(float x, float y) {
        ...
        drawable.getPaint().setColor(mTManager.colorBGCircle());//背景颜色 圆圈
        return circle1;
    }
    ...
    public void defineRegion(final int x, final int y) {
        ...
        for (int i = 0; i < tmp.length; i++) {
            for (int j = 0; j < tmp[i].length; j++) {
                //设置是否选中
                mCManager.getDateInfo(centerYear, centerMonth, false)[i][j].isChoosed = false;
                ...
                if (region.contains(x, y)) {
                    ...
                    //设置是否选中
                    mCManager.getDateInfo(centerYear, centerMonth, false)[i][j].isChoosed = true;
